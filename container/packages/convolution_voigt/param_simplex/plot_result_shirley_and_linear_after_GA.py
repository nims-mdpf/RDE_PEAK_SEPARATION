...
# 指定したディレクトリ dnm 以下にある "summary_evalFunc_vs_ll.csv" を使い
...
def get_prefered_result_by_BIC(dnm, figLevel=2, targetKey="bic_gauss"):
    ...
    """
    fnm = dnm + "summary_evalFunc_vs_ll.csv"
    if not os.path.exists(fnm):
        print("Not found", fnm)
        return

    df3 = pd.read_csv(fnm, usecols=["numPeak", "method", "ID", targetKey])
...
def myplot_auto_smoothing(csvfile):
    ...
    myplot_simple(df2[df2.columns[0]], [df2[df2.columns[1]]], "ID", "Number of peaks", "as_NumPeaks.png")
    myplot_simple(df2[df2.columns[0]], [df2[df2.columns[3]]], "ID", "chi^2", "as_chi2.png", logy=True)
    myplot_simple(df2[df2.columns[0]], [df2[df2.columns[4]]], "ID", "Reduced chi^2", "as_chi2_reduced.png", logy=True)
    ...
    myplot_simple(df2[df2.columns[0]], yList, "ID", "AIC, BIC", "as_aic_bic.png", legends=legendList)
    ...
    myplot_simple(df2[df2.columns[0]], yList, "ID", "cAIC", "as_cAIC.png", legends=legendList)
    myplot_simple(df2[df2.columns[1]], yList, "Number of peaks", "cAIC", "as_cAIC_vs_NumPeak.png", legends=legendList)

    myplot_simple(df2[df2.columns[0]], [df2[df2.columns[9]]], "ID", "Minimum peak HWHM (eV)", "as_MinWidth.png")
    # myplot_simple(df2[df2.columns[0]], [df2[df2.columns[10]]], 'ID', 'Maximum peak HWHM (eV)', 'as_MaxWidth.png')
...
def myplot_auto_smoothing_minpoints(csvfile):
    ...
    myplot_simple(df2[df2.columns[0]], [df2[df2.columns[1]]], "ID", "Minimum points position (eV)", "as_minpoints.png")
    return
...
def get_prefered_result(df2, key1, key2):
    ...
    values = [key2, method, ll, df2["numPeak"][m], df2[key1][m]]

    fname1 = f"./{method}/result_peak_search_ll{ll:04d}.png"
    fname2 = f'./prefered_result_by_{key2}_{method}_ll{ll:04d}_numPeak{df2["numPeak"][m]}.png'
    if os.path.exists(fname1):
        print("Copy from %s to %s" % (fname1, fname2))
        shutil.copy2(fname1, fname2)
    else:
...
def myplot_aic_bic(idHigherS, idLowerS, paper=False, figLevel=2):
    ...
    for fitting_result_file in glob("./*/result_peak_search_ll*.csv"):
        fitting_parameter_file = fitting_result_file.replace("result", "parameters")
        match = re.search(r"ll(.*).csv", fitting_result_file)
    ...
    # print(df2)
    df2.to_csv("./summary_evalFunc_vs_ll.csv")
    ...
    columns4 = ["function_to_be_minimize", "method", "ll", "numPeak", "value"]
    ...
    # 各種プロット
    myplot_simple(df2["ID"], [df2["numPeak"]], "ID", "Number of peaks", "py_NumPeaks.png")
    # myplot_simple(df2["smoothing_point"], [df2["chi2"]], 'ID', 'chi^2', 'py_chi2.png', logy = True)
    # myplot_simple(df2["smoothing_point"], [df2["reduced_chi2"]], 'ID', 'Reduced chi^2', 'py_chi2_reduced.png', logy = True)
    ...
    # yList = [df2[df2.columns[12]], df2[df2.columns[13]]]
    # legendList = ['cAIC(Poisson)', 'cAIC(Gauss)']
    # myplot_simple(df2["smoothing_point"], yList, 'ID', 'cAIC', 'py_cAIC.png', legends = legendList)
    ...
    myplot_simple(df2["ID"], yList, "ID", "Energy range (eV)", "py_energyRange.png", legends=legendList)
...
#     axes[0].plot(df1['ll'], df1['RMSPE'], 'o')
#     axes[0].ticklabel_format(style="plain", axis="y")
#     axes[0].set_ylim(ylim)
#     axes[0].set_xlabel("Initial model ID")
...
def plot2Dhist_BIC(targetDir=".", binsX=None, binsY=100, rangeX=None, rangeY=None, targetKey="bic_gauss"):
    csvfile = f"{targetDir}/summary_evalFunc_vs_ll.csv"
    # print(csvfile)
    ...
    pngfile = csvfile.replace("summary_evalFunc_vs_ll.csv", f"{targetKey}_vs_NumPeak_hist2D.png")
...
def plot_result(idHigherS, idLowerS, multi=0, globkey="./*/result_peak_search_ll*.csv", paper=False, ylim=None, figLevel=2):
