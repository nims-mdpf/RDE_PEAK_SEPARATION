...
activeshirley_exe_file = f"{bindir}/activeshirley_gcc.exe"
plot_py_file = f"{bindir}/plot_result_shirley_and_linear.py"
...
def get_num_model(kwd):
    ...
    numModel = len(paramfileList)
    return numModel


def main_core(fnm, kwd, noiseType="-g", devFlag=False):
    # pwd = os.getcwd()
...
def main(fnm, bgType="auto", noiseType="-g", figType="-minfig", devFlag=False):

    if bgType == "auto":
        numModel = main_core(fnm, "shirley", noiseType=noiseType, devFlag=devFlag)

        if numModel == 0:
            numModel = main_core(fnm, "linear", noiseType=noiseType, devFlag=devFlag)
...
def show_usage():
    print(f"Usage:\n        {args[0]} <csv file> [-d] [-s|-l] [-allfig|-minfig|-nofig] [-g|-p]")
...
if __name__ == "__main__":
    ...
    main(args[1], bgType, noiseType, figType, devFlag)
